...
class Map:
    ...
    def setup_sidebar_ui(self):
        ...
        tk.Label(edge_info, textvariable=self.info_traffic, bg="#eff6ff", fg=TEXT_DARK, anchor=tk.W,
                 font=("Arial", 9, "bold")).pack(fill=tk.X, pady=(0, 4))
        tk.Label(edge_info, textvariable=self.info_type, bg="#eff6ff", fg=TEXT_DARK, anchor=tk.W).pack(fill=tk.X,
                                                                                                       pady=2)
        tk.Label(edge_info, textvariable=self.info_length, bg="#eff6ff", fg=TEXT_DARK, anchor=tk.W).pack(fill=tk.X,
                                                                                                         pady=2)

        search_info = self.make_card(self.sidebar, "Search Stats", bg="#f0fdf4")
        search_info.pack(fill=tk.X, pady=(0, 10))

        tk.Label(search_info, textvariable=self.status_var, bg="#f0fdf4", fg=TEXT_DARK, anchor=tk.W,
                 wraplength=200).pack(fill=tk.X, pady=2)
        tk.Label(search_info, textvariable=self.cost_var, bg="#f0fdf4", fg=TEXT_DARK, anchor=tk.W).pack(fill=tk.X,
                                                                                                        pady=2)
        tk.Label(search_info, textvariable=self.expanded_var, bg="#f0fdf4", fg=TEXT_DARK, anchor=tk.W).pack(fill=tk.X,
                                                                                                            pady=2)
        tk.Label(search_info, textvariable=self.runtime_var, bg="#f0fdf4", fg=TEXT_DARK, anchor=tk.W).pack(fill=tk.X,
                                                                                                           pady=2)
    ...
    def redraw_selection_labels(self):
        ...
        for role, node in (("S", self.selected_start), ("G", self.selected_goal)):
            ...
            label_color = self.NODE_MARKER_STYLES["start"]["fill"] if role == "S" else self.NODE_MARKER_STYLES["goal"][
                "fill"]
