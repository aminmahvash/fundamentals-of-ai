    ...
    for depth_limit in range(0, 51):
        ...
        while stack:
            ...
            for next_state, edge_cost in successors:
                ...
                if env.is_goal_state(next_state):
                    print(f"Total expanded nodes: {total_expanded + expanded_this}")
                    print(f"Path length: {len(new_path)}")
                    print(f"Total path cost: {new_cost}")
                    yield ("frontier", next_state)
                    yield ("goal", new_path)
                    return new_path
    ...
    while queue_f and queue_b:
        ...
        if queue_f:
            ...
            if state_f in visited_b:
                path_b, cost_b = visited_b[state_f]
                full_path = path_f + path_b[-2::-1]
                full_cost = cost_f + cost_b
                print(f"Expanded nodes: {expanded_count}")
        ...
        if queue_b:
            ...
            if state_b in visited_f:
                path_f, cost_f = visited_f[state_b]
                full_path = path_f + path_b[-2::-1]
                full_cost = cost_f + cost_b
                print(f"Expanded nodes: {expanded_count}")
                print(f"Path length (nodes): {len(full_path)}")
